...
public class CreatureModifiersEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        Modifiers mod_mask = SetModMask(mod_property);
        mod_mask = (Modifiers)EditorGUILayout.EnumMaskField("Mods: ", mod_mask);

        mod_property.ClearArray();
        string display_mods = "";

        foreach (Modifiers mod in System.Enum.GetValues(typeof(Modifiers))) {
            if (((int)mod_mask & (1 << ((int)mod - 1))) != 0 && System.Enum.IsDefined(typeof(Modifiers), (int)mod)) {
        ...
        if (HasMod(Modifiers.battlecry, (int)mod_mask)) {
        ...
        if (HasMod(Modifiers.overload, (int)mod_mask)) {
    ...
    private Modifiers SetModMask(SerializedProperty property) {
        Modifiers to_return = 0;
    ...
    static bool HasMod(Modifiers mod, int mod_mask) {
